...
- Compensation exposes every canonical Retail salary row to the authenticated users through a complete versioned read-model: stable person/agent identity, display name, company, store and organizational dimensions, total and available provenance. KPI, person detail, filters, inspect and export use the same rows. There is no cohort suppression, minimum-person threshold, identity masking or exclusion merely because a legacy row lacks a newer import-batch approval field. CNP never enters the Insight read-model, API, export or application logs.
...
- Planning forecast identifies run, horizon, method/model, input cutoff and coverage. Head-ul promovat indică varianta oficială cu hash/row-count/revision/ledger, dar run-urile candidate, drafturile și scenariile legacy existente rămân vizibile și clar etichetate pentru analiză. Target scenarios identify scenario, revision, status, rule-set hash and snapshot provenance. Accuracy păstrează KPI-ul server-side pentru varianta selectată; clientul nu recalculează o formulă alternativă.
...
Retail migration 047 is the historical aggregate-only baseline. Migrations 059–061 supersede it aditiv cu snapshot v7, Compensation v2 complet și Finance v2 actual/estimat, păstrând v1/v6 ca ancore N-1 și optimizând filtrarea pe perioadă. Applied migration files remain immutable. Legacy raw grants do not authorize arbitrary SQL, but complete versioned read-models expose all available business data to authenticated users.
