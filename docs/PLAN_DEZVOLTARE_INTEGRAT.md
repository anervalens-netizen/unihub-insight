...
La 2026-08-07, Retail păstrează aditiv contractele anterioare și publică prin migrările 057–058 snapshot v6, Campaigns v3 cu variantă canonică, Concurs v1 materializat exclusiv din `ContestsService` și Grile v2. Auditul owner a constatat că RC3 ascunde nejustificat date Retail: Compensation v1 elimină persoanele și rândurile legacy fără batch aprobat, iar Finance v1 cere generation head și publică numai actuale. Acestea sunt defecte deschise, nu stări corecte de produs. Politica țintă este vizibilitate completă și identică pentru owner și directorul general, cu provenance `actual/estimated/legacy/draft` afișată explicit. Închiderea `1.0.0` cere corectarea contractelor/runtime, allowlistul real cu doi utilizatori, acceptarea vizuală owner și șapte zile curate pe exact SHA-ul final.
...
| Acces | PARȚIAL | Authentik este live; trebuie reconciliat allowlistul exact owner + director general și eliminată separarea HR/P&L/capabilități dintre ei |
...
| Custom Dashboards | RC1 | CRUD/preset/clone/duplicate/layout/versionare/ACL/batch sunt implementate; ACL controlează layoutul, nu datele; lipsește matricea live cu cele două sesiuni autorizate |
...
| Campanii | Focus, Promo/Incentive/Folii prin `reporting_campaign_month_v3`; Concurs prin `reporting_contest_month_v1` | Insight consumă variante și punctaj canonic; premiul rămâne etichetă, iar unitățile Promo nu sunt redenumite bonuri |
| Workforce și Grile | lifecycle/profil comercial, `reporting_grile_month_v2`; Visits v2 FieldOps | Visits rămâne pe autor Team Leader; Workforce este explicit activitate comercială observată, iar Grile alege o singură generație eligibilă pentru lună, nu roster sau closeout inventat |
| Compensații | 3.716 înregistrări lunare 2025-01…2026-06 la snapshot și legături agent-persoană | se expun toate persoanele, valorile/componentele salariale, ierarhia și provenance; fără cohort suppression sau condiții de batch care ascund rânduri acceptate de Retail |
| Finance/P&L | `store_pnl_monthly`, 2017-01…2026-06 la snapshot; tabelele noii autorități de generații nu au încă head live | se afișează aceleași rânduri ca Retail, cu `actual/estimated`, authority, reconciliere și coverage; lipsa head-ului nu golește modulul |
| Planning | forecast lunar 2026-07…2027-07 și scenarii Target 2026-06…2026-08 la snapshot | toate run-urile/scenariile disponibile sunt vizibile cu status/provenance; head-ul definește varianta oficială, nu ascunde candidații de utilizatorii autorizați |
...
- `reporting_campaign_month_v3`: mecanism și variantă canonică, campanie, perioadă/cutoff, firmă/RM/magazin/agent/produs, vânzări, cantitate netă semnată, discount/recompensă, eligibilitate, coduri active și statut `official/partial/unavailable`;
- `reporting_contest_month_v1`: concurs, identitate publică agent×magazin, unități/puncte pe regulă, clasament, premiu textual, coverage, warnings și head immutable;
- `reporting_grile_month_v2`: status per magazin dintr-o singură sursă lunară aleasă de snapshot v6, cu generation/authority, coverage și erori păstrate;
...
- `reporting_compensation_person_month_v2`: persoană/agent, nume, firmă, ierarhie/magazin, valori și componente salariale, source/provenance și agregate conservate; CNP sau alte câmpuri sursă rămân disponibile în detail/export autentificat când sunt necesare;
...
- Compensation publică prin read-model versionat toate rândurile canonice Retail și identitatea persoanei, fără cohort suppression, minimum-person gate, identity masking sau condiția ca rândul legacy să aibă `import_batch_id`. API/filtre/inspect/export conservă aceleași rânduri; orice medie cu prag de 2.000 RON este o metrică separată și etichetată, nu înlocuitorul mediei tuturor persoanelor;
- Finance selectează explicit `actual` versus `estimated` la cheia companie–lună–magazin canonic; `__FINANCE_UNALLOCATED__` intră în total și are propriul detail. Shadow/draft rămâne vizibil și etichetat, fără a deveni actual;
- Planning fixează `run_id`, horizon, model/method, input cutoff, coverage, scenario/revision/rule-set și status. Head-ul identifică varianta oficială, dar candidații/drafturile disponibile pot fi analizate explicit și nu sunt ascunse.

`UNAVAILABLE` se folosește numai când Retail nu are rânduri canonice pentru scope/perioadă sau sursa este inaccesibilă. `PARTIAL` arată coverage/provenance incomplet și păstrează vizibile rândurile existente; lipsa unui contract/head nou nu mai este motiv suficient pentru ecran gol.

## Experiența finală

| Modul | Suprafețe obligatorii |
| --- | --- |
| Overview | health în 10 secunde, target/forecast/comparații, profit și cost salarial, creșteri/scăderi, risc, alerte de date, abateri și explicații evidence-backed |
...
| Compensation | persoane și toate valorile/componentele disponibile, structură fix/variabil/bonuri, medie/mediană/distribuție, payroll/sales, payroll/profit și performanță versus remunerație, fără suprimare |
...
| Distribuție | histogramă + boxplot | violin numai cu eșantion suficient și metodologie explicată | arată `n`, mediană, quartile și outliers; persoanele și backing table-ul complet rămân disponibile |
...
Stare tehnică RC1: adaptorul Canvas înregistrează modular Line, Bar, Pie/Donut, Scatter, Heatmap, Boxplot, Treemap și Calendar; Area, histogram, waterfall reconciliat și forecast band sunt compoziții controlate în `ChartSpec`. Orice formă fără dimensiunile semantice cerute revine la tabel, iar waterfall-ul este refuzat dacă start + delta nu reconciliază totalul. Calendarul Sales este oferit peste `reporting_sales_day_v1`: rândurile sunt zile observate, lipsa nu devine zero, iar returul este cantitate negativă. Transactions păstrează cele trei agregate canonice de bon și trebuie să adauge detailul bon/linie disponibil în Retail; distribuția comută histogramă ↔ boxplot peste același eșantion și păstrează backing table-ul complet chiar dacă boxplot-ul cere `n>=5`; Planning Accuracy folosește perechile server-side Actual × Forecast pe magazin. Funnel, Sankey/Chord, Brush, SVG și violin rămân `NEIMPLEMENTAT` până la un caz de business și un POC măsurat; forecast band nu este oferit fără intervale autoritative.
...
- resolver de `analytical_snapshot_id` înaintea catalogului: toate widgeturile unui dashboard citesc același snapshot/cutoff coerent; `completed/promoted` identifică autoritatea preferată, iar rândurile canonice legacy/estimate/draft rămân disponibile și etichetate, cu compatibilitate Consumer N/N-1;
...
- catalog versionat pentru metrici, dimensiuni, grain-uri, comparații, formule, unități, missing policy și effective dates; catalogul nu ascunde date între cei doi utilizatori autorizați;
...
- `private`, `shared read-only` și permisiune explicită per subject pentru layout; ambii utilizatori autorizați au aceeași vizibilitate completă asupra datelor indiferent de ACL-ul dashboardului;
...
- exporturile aplică allowlistul aplicației și includ aceleași rânduri complete ca API-ul, fără suprimare de persoană/cohortă.
...
- accesul exterior rămâne Authentik cu allowlist exact owner + director general; amândoi primesc aceeași vizibilitate Analytics/Management/Compensation/Finance/Planning/Admin;
- E2E de identitate folosește forma reală Authentik și verifică cei doi subiecți allowlistați plus un subiect neautorizat;
- browserul nu primește DB credentials sau SQL. Datele business, inclusiv identitatea salarială/CNP când este cerută de contractul de detail, sunt livrate numai după autentificare și nu intră în loguri;
...
**Acceptanță:** allowlistul real, vizibilitatea completă identică pentru cei doi utilizatori, public boundary, performanța, backup/restore și rollback trec pe exact același SHA.
...
- allowlist matrix cu acces complet pentru owner/director și 403 pentru un subiect neallowlistat, verificată direct pe endpoint și export;
...
- nu se suprimă persoane, cohorte sau câmpuri business de la utilizatorii allowlistați; CNP nu se scrie în loguri;
- nu se prezintă datele shadow/draft/estimated drept actuale Finance, dar ele rămân vizibile cu eticheta corectă;
...
- ownerul și directorul general văd toate datele în UI/inspect/export, iar accesul neallowlistat și public boundary trec matricea negativă;
