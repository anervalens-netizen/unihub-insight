...
| Inspect/export | PARȚIAL | XLSX pe Overview/module/raport lunar și CSV în inspectorul modulelor; lipsesc PNG și un contract unificat per widget |
| Interacțiuni analitice | PARȚIAL | fullscreen, sortare și inspect pe module; lipsesc cross-filter, breadcrumb, drill-down complet și selectarea intervalului din grafic |
...
Baseline tehnic real: adaptorul înregistrează astăzi numai Line, Bar, Pie/Donut, Scatter și Heatmap, exclusiv Canvas; Area și unele Combo/Waterfall sunt compoziții peste Line/Bar. Boxplot, Treemap, Calendar/Matrix, Funnel, Sankey/Chord, Brush, SVG și seriile custom de interval/violin sunt `NEIMPLEMENTAT` până la POC și înregistrarea modulară. Catalogul va marca fiecare opțiune `native`, `composed` sau `custom`; un label „waterfall” nu este acceptat dacă seria redată este doar un bar fără bridge reconciliabil.
...
**POC-uri înainte de propagarea catalogului:** dashboard cu 8–12 widgeturi la 1180/1440/1920/ultrawide; trend 24–36 luni cu zoom; heatmap aproximativ 74×24; scatter până la 5.000 puncte Canvas, cu agregare/sampling server-side peste limită; click/double-click/blank-reset cu URL și reload; light/dark + Compact/Comfortable + ARIA/decals; PNG la pixel ratio controlat; bundle/chunk, first render, resize, frame time și memorie pentru Canvas/SVG fără creștere după navigări repetate. Fiecare chart acceptat este înregistrat modular și încărcat numai în chunkul necesar. Poarta rămâne interacțiune p95 sub 200 ms.
...
- reconciliere la cent pentru Finance și la unitate/bon pentru Sales/Campaigns;
