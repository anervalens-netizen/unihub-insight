...
Latest focused evidence waits for the ECharts `finished` event rather than canvas insertion: first render 5,753.5 ms, resize blocking p95 33.5 ms and heap growth -2,075,992 bytes after three remounts, all inside the 8,000 ms / 200 ms / 64 MiB gates. Canvas remains the supported renderer; this synthetic pass does not authorize SVG and does not count toward the production RUM gate.

Reproducible functional browser coverage runs with `npm run browser:qa`; it includes the histogram↔boxplot same-sample toggle, distinct Retail detail shortcut, filter-preset lifecycle, targeted ACL editing and atomic widget duplication. Functional browser QA does not replace the production RUM/load gate or owner visual acceptance.
