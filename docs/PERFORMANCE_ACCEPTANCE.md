...
Latest focused evidence on the specialization candidate: first render 7,634.2 ms, resize blocking p95 151.5 ms and heap growth 18,927,354 bytes after three remounts, all inside the 8,000 ms / 200 ms / 64 MiB gates. Canvas remains the supported renderer; this synthetic pass does not authorize SVG and does not count toward the production RUM gate.

Reproducible functional browser coverage runs with `npm run browser:qa`; the current candidate passes 51/51, including the histogram↔boxplot same-sample toggle and the distinct Retail detail shortcut. It does not replace the production RUM/load gate or owner visual acceptance.
