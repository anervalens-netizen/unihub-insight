...
- [ ] Read-model-uri Retail sunt publicate aditiv; Sales Calendar folosește view-ul zilnic reconciliat, Visits v2 păstrează autorul Team Leader, iar migrările 057–058 adaugă Campaigns v3, Concurs v1, Grile v2 și snapshot v6 fără a rupe N-1. Compensation și Finance necesită contracte complete noi care expun aceleași rânduri acceptate de Retail; head-ul/provenance îmbogățește datele, nu le ascunde.
...
- [ ] Corecție Compensation: persoane, valori/componente salariale, ierarhie și provenance complete în KPI/detail/inspect/export; fără prag de cohortă, mascarea identității sau condiția legacy `import_batch_id` care ascunde rândurile Retail.
...
- Finance și Compensation sunt incorect `UNAVAILABLE` pentru lunile unde Retail are date. Cauza confirmată: Compensation v1 cere batch aprobat și elimină persoanele; Finance v1 cere generation head și exclude estimările. Ambele trebuie supersedate aditiv.
...
- Promovarea `1.0.0` mai cere Compensation/Finance complete, corectarea statusurilor `partial`, allowlistul real owner + director general, cazurile edge lipsă, acceptarea vizuală owner și șapte zile curate conform [Performance Acceptance](docs/PERFORMANCE_ACCEPTANCE.md).

## Restricții runtime confirmate de eliminat
