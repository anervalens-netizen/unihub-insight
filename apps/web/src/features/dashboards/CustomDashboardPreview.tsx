...
export function CustomDashboardPreview({ dashboard, search, editMode, resetToken, onLayoutChange }: { dashboard: DashboardDocument; search: GlobalSearch & { period: string }; editMode: boolean; resetToken: number; onLayoutChange: (items: DashboardLayoutItem[]) => void }) {
  const queryInputs = useMemo(() => dashboard.widgets.map((widget) => resolveWidgetSearch(search, widget)), [dashboard.widgets, search]);
  const queries = useQueries({ queries: dashboard.widgets.map((widget, index) => moduleAnalyticsQuery(widget.module, queryInputs[index] ?? search)) });
  ...
  if (firstError?.error) return <ErrorState message={firstError.error instanceof Error ? firstError.error.message : 'Un card nu a putut fi încărcat.'} />;
  const definitions: DashboardWidgetDefinition[] = dashboard.widgets.map((widget: DashboardWidget, index) => {
