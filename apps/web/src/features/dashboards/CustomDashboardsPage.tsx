...
import { dashboardTemplates, moduleMetrics } from './templates';
import { WidgetEditorRow } from './WidgetEditorRow';

const moduleCapability: Record<ModuleId, Capability> = { sales: 'insight:analytics', performance: 'insight:analytics', campaigns: 'insight:analytics', workforce: 'insight:management', compensation: 'insight:hr', finance: 'insight:pnl', planning: 'insight:management' };
function cloneDocument(document: DashboardDocument): DashboardDocument { return structuredClone(document); }
function nextWidget(module: ModuleId, existing: DashboardWidget[]): DashboardWidget { const metric = moduleMetrics[module][0]; const y = existing.reduce((maximum, item) => Math.max(maximum, item.layout.y + item.layout.h), 0); return { id: crypto.randomUUID(), module, title: metric?.label ?? 'Widget nou', metric_id: metric?.id ?? 'sales.total', visualization: 'kpi', dimension: null, time_grain: 'month', filter_mode: 'inherit', filters: {}, options: {}, layout: { x: 0, y, w: 6, h: 5, min_w: 4, min_h: 4 } }; }
...
export function CustomDashboardsPage() {
  ...
  const availableModules = useMemo(() => (Object.keys(moduleMetrics) as ModuleId[]).filter((module) => identity.capabilities.includes(moduleCapability[module])), [identity.capabilities]);
  const availableTemplates = dashboardTemplates.filter((template) => template.requiredCapabilities.every((capability) => identity.capabilities.includes(capability)));
